...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;Rutina que escribe una cadena por el LCD;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Escribe la cadena apuntada en DPTR, y MODIFICA dicho puntero
PUTLCD:
	CLR	A
	MOVC	A,@A+DPTR	; Lee del segmento de codigo
	JZ	EXIT2		; Sale si el caracter es 00H
	CALL	WRITE_TEXT	; Escribimos el caracter en el LCD
	INC	DPTR		; Incrementamos el contador
	SJMP	PUTLCD
EXIT2:	RET
...
PRINT_R:
	...
	CALL	WRITE_TEXT	; Escribimos en el LCD
	MOV	A,B
...
PRINT_C:
	CALL    CLEAR_LCD       ; Limpiamos el LCD
...
OK_C:	MOV	R7,B		; Restauramos los segundos
	POP	B		; Recuperamos B
	RET
...
REP1:
	...
	MOV	DPTR,#MSG1
	CALL	PUTLCD		; Escribimos en el LCD
...
CHECK:	
	MOV	B,R3	
	JB	B.1,STOP
	SJMP	CHECK    	; Si no estamos seguimos esperando
...
REP4:
	...
	CALL	INCR_C		; Incrementamos centesimas
        JBC	FLAG,P_LCD
	SJMP	REP4		; Sino ha cambiado el segundo no lo mostramos	
P_LCD:	CALL    CLEAR_LCD       ; Limpiamos el LCD
...
MSG1:	DB	'RELOJ/CRONOMETRO (R/C):',00H
MSG2:	DB	'MODO: ',00H
MSG3:	DB	'MODO CRONOMETRO',00H
MSG4:	DB	'MODO RELOJ',00H
MSG5:	DB	'HORAS (HH):',00H
MSG6:	DB	'MINUTOS (MM):',00H
MSG7:	DB	CR,LF,00H
MSG8:	DB	'HORA: ',00H
MSG9:	DB	CR,00H
